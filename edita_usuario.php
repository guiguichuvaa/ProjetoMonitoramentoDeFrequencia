...
$is_autorizedEditado = mysqli_real_escape_string($conexao, $_POST['is_autorized']);

// Verificar se a senha foi preenchida
if (!empty($_POST['senha'])) {
    $senhaEditada = mysqli_real_escape_string($conexao, md5($_POST['senha']));
    $query = "UPDATE usuario SET nome_usuario = '$nomeEditado', email_usuario = '$emailEditado', senha_usuario = '$senhaEditada', perfil_usuario = '$perfilEditado', is_autorized = '$is_autorizedEditado' WHERE id_usuario = '$id_usuario'";
} else {
    // Se a senha não foi preenchida, não atualizar a senha
    $query = "UPDATE usuario SET nome_usuario = '$nomeEditado', email_usuario = '$emailEditado', perfil_usuario = '$perfilEditado', is_autorized = '$is_autorizedEditado' WHERE id_usuario = '$id_usuario'";
}
