...
if ($row == 1) {
    ...
    // Se o email do usuario tiver @admin.com, redireciona para o painel de admin, mas se tiver com @coletor.com
    // redireciona para o painel do coletor
    if (str_ends_with($email, '@admin.com')) {
        ...
        exit();
    } elseif (str_ends_with($email, '@coletor.com')) {
        $_SESSION['id_usuario'] = $userData['id_usuario'];
        $_SESSION['email'] = $userData['email_usuario'];
        $_SESSION['nome_usuario'] = $userData['nome_usuario'];
        $_SESSION['perfil_usuario'] = $userData['perfil_usuario'];
        header('Location: lista_frequencia_coletor.php');
        exit();
    } 
} else {
